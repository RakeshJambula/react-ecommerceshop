...
const AddressPage = () => {
  ...
  // Check if coming from cart checkout (so redirect to order page later)
  ...
  const handSubmit = async (e) => {
    ...
    try {
      ...
      // 🎯 Redirect logic
      if (isFromCheckout) {
        navigate("/order", { state: { from: "address" } });
      } else {
        navigate("/profile");
      }
    } catch (error) {
      setError(
        error.response?.data?.message || "Failed to save/update address"
      );
    }
  };
